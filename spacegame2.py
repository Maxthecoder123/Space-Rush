...
playerRocket = pygame.image.load("Player.png")
star = pygame.image.load("Star.png")
asteroid50 = pygame.image.load("smallAsteriod.png")
asteroid100 = pygame.image.load("largeAsteriod.png")
asteroid150 = pygame.image.load("megaAsteriod.png")

shoot = pygame.mixer.Sound("shoot.wav")
bangLargeSound = pygame.mixer.Sound("bangLarge.wav")
bangSmallSound = pygame.mixer.Sound("bangSmall.wav")
pygame.display.set_caption("Asteroid Rush")
...
class Bullet(object):
    ...
    def draw(self, win):
        pygame.draw.rect(win, (255, 255, 255), (self.x, self.y, 4, 4))
    ...
    if not gameover:
        ...
        for a in asteroids:
            ...
            if (a.x>=player.x-player.w//2 and a.x<=player.x+player.w//2) or (a.x + a.w <= player.x - player.w//2 and a.x + a.w >= player.x - player.w//2):
                if (a.y >= player.y - player.h//2 and a.y <= player.y + player.h//2) or (a.y + a.h >= player.y//2 and a.y + a.h <= player.y + player.h//2):
                    lives -= 1
        ...
        # bullet collision
            for b in playerBullets:
                if (b.x >= a.x and b.x <= a.w) or b.x + b.w >= a.x and b.x + b.w <= a.x + a.w:
                    if (b.y >= a.y and b.y <= a.y + a.h) or b.y + b.h >= a.y and b.y + b.h <= a.y + a.h:
                        if a.rank ==3:
                            if isSoundOn:
                                bangLargeSound.play()
